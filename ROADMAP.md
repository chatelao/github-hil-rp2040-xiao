...
- [ ] **Task 1.1: Environment & Repository Structure Setup**
  - [ ] Create Python project structure (`src/xiao_flasher/`, `test/`) with `pyproject.toml` / `setup.py`.
  - [ ] Implement `src/install.sh` for runtime installation setup.
  - [ ] Implement `test/install.sh` for installing test dependencies (`pytest`, `pytest-cov`, `mypy`, `ruff`).
